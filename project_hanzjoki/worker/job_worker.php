        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    ...
                    <div class="atas-bgn">
                                    <a href="takejob.php" class="jobdesk">
                                        <span>JOB</span>
                                    </a>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
                ...
                <a href='#' class='btn btn-success btn-done' data-id='{$row['id_transaksi']}'>Done</a>
                <a href='#' class='btn btn-info btn-laporan' data-id='{$row['id_transaksi']}'>Laporan</a>

                <button class='btn btn-danger btn-detail-akun' data-id='{$row['id_transaksi']}'>Akun</button>
...
<script>
    $(document).ready(function () {
        // Tindakan ketika tombol "Problem" ditekan
        $('.btn-problem').on('click', function (e) {
            e.preventDefault();

            // Ambil id_transaksi dari atribut data-id
            var id_transaksi = $(this).data('id');

            // Kirim data ke update_problem.php menggunakan AJAX
            $.ajax({
                type: 'POST',
...
<div class="modal fade" id="popup" tabindex="-1" role="dialog" aria-labelledby="popupTitle" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="popupTitle">Data akun</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
$(document).ready(function () {
    ...
    function showPopup(id_transaksi) {
        // Ganti 'popup_detail_order.php' dengan file yang berisi detail popup Anda
        $.post('detail_akun.php', { id_transaksi: id_transaksi }, function (data) {
            $('#popupContent').html(data);
            $('#popup').modal('show');
        });
    }
    ...
    // Event handler untuk tombol Detail
    $('.btn-detail-akun').click(function () {
        // Ambil nilai id_transaksi dari atribut data-id
        var id_transaksi = $(this).data('id');
